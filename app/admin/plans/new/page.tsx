...
const DEFAULT_STRUCTURE = JSON.stringify([
  { week: 1, sessions: [{ day: '월', type: 'easy', distance_km: 5, description: '가벼운 조깅' }] }
], null, 2)
...
export default function NewPlanPage() {
  ...
  const [weeklyStructure, setWeeklyStructure] = useState(DEFAULT_STRUCTURE)
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-black text-white">새 트레이닝 플랜</h1>
      </div>

      <form onSubmit={handleSubmit} className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-6 space-y-5">
        <Field label="플랜 이름">
          <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className={inputCls} placeholder="5K 입문 플랜" />
        </Field>
        ...
        <Field label="설명">
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className={inputCls + ' resize-none'} placeholder="플랜 소개" />
        </Field>
