  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-black text-white">새 코스</h1>
      </div>

      <form onSubmit={handleSubmit} className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-6 space-y-5">
        <Field label="코스명">
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className={inputCls} placeholder="한강공원 러닝 코스" />
        </Field>
        <Field label="위치">
          <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} required className={inputCls} placeholder="서울시 영등포구" />
        </Field>
        <div className="grid grid-cols-3 gap-4">
          <Field label="거리 (km)">
            <input type="number" value={distanceKm} onChange={(e) => setDistanceKm(e.target.value)} required step="0.1" min="0" className={inputCls} placeholder="10.0" />
          </Field>
          <Field label="난이도">
        ...
        <Field label="설명">
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} className={inputCls + ' resize-none'} placeholder="코스 특징 및 상세 설명" />
        </Field>
        <Field label="지도 URL (카카오맵 등)">
          <input type="url" value={mapEmbedUrl} onChange={(e) => setMapEmbedUrl(e.target.value)} className={inputCls} placeholder="https://..." />
        </Field>
