...
export default function EditColumnPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/columns/${id}`).then((r) => r.json()).then((data) => {
      setTitle(data.title ?? '')
      setSlug(data.slug ?? '')
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-black text-white">칼럼 수정</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-6 space-y-5">
        ...
        <Field label="본문">
          <textarea value={content} onChange={(e) => setContent(e.target.value)} required rows={14} className={inputCls + ' resize-y'} />
        </Field>
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => setPublished(!published)}
            className={`w-10 h-6 rounded-full transition-colors flex items-center ${published ? 'bg-[#FF4D00]' : 'bg-white/10'}`}>
            <div className={`w-4 h-4 bg-white rounded-full mx-1 transition-transform ${published ? 'translate-x-4' : 'translate-x-0'}`} />
          </button>
          <span className="text-white/60 text-sm">{published ? '발행됨' : '미발행'}</span>
        </div>
