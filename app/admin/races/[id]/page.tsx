...
export default function EditRacePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/races/${id}`).then((r) => r.json()).then((data) => {
      setTitle(data.title ?? '')
      setDate(data.date ?? '')
      setLocation(data.location ?? '')
      setDistances((data.distances ?? []).join(', '))
      setRegStart(data.registration_start ?? '')
      setRegEnd(data.registration_end ?? '')
      setUrl(data.url ?? '')
      setFetching(false)
    })
  }, [id])
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-black text-white">대회 수정</h1>
      </div>
